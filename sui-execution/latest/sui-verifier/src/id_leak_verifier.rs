...
fn execute_inner(
    ...
    match bytecode {
        ...
        Bytecode::PackVariant(vidx) =>  {
            let handle = expect_ok(verifier.binary_view.variant_handle_at(*vidx))?;
            let variant = expect_ok(verifier.binary_view.variant_def_at(handle.enum_def, handle.variant))?;
            let num_fields = variant.fields.len();
            verifier.stack_popn(num_fields as u64)?;
            verifier.stack_push(AbstractValue::Other)?;
        }
        Bytecode::PackVariantGeneric(vidx) =>  {
            let handle = expect_ok(verifier.binary_view.variant_instantiation_handle_at(*vidx))?;
            let enum_inst = expect_ok(verifier.binary_view.enum_instantiation_at(handle.enum_def))?;
            let variant = expect_ok(verifier.binary_view.variant_def_at(enum_inst.def, handle.variant))?;
            let num_fields = variant.fields.len();
            verifier.stack_popn(num_fields as u64)?;
            verifier.stack_push(AbstractValue::Other)?;
        }
