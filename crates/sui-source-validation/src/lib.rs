...
/// Ensures dependent modules in `deps_compiled_units` are compiled with the
/// compiler version in Move.lock. This works by detecting if a compiled unit
/// requires a prior compiler version:
...
fn units_for_toolchain(
    deps_compiled_units: &Vec<(PackageName, CompiledUnitWithSource)>,
